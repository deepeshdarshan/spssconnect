/**
 * @fileoverview Malayalam locale strings for the SPSS Connect data entry form.
 * @module locales/ml
 */

const ml = {
  // Page
  'page.title': 'പുതിയ രേഖ ചേർക്കുക',
  'page.editTitle': 'രേഖ എഡിറ്റ് ചെയ്യുക',

  // Sections
  'section.personalDetails': 'വ്യക്തിഗത വിവരങ്ങൾ (വീട്ടുടമ)',
  'section.membershipDetails': 'അംഗത്വ വിവരങ്ങൾ',
  'section.address': 'വിലാസം',
  'section.healthFamily': 'കുടുംബവും ക്ഷേമവും',
  'section.members': 'അംഗങ്ങളുടെ വിവരങ്ങൾ',
  'section.nonMembers': 'അംഗമല്ലാത്തവരുടെ വിവരങ്ങൾ',

  // Sub-sections (dynamic blocks)
  'subsection.basicDetails': 'അടിസ്ഥാന വിവരങ്ങൾ',
  'subsection.membershipInfo': 'അംഗത്വ വിശദാംശങ്ങൾ',
  'subsection.contactDetails': 'ബന്ധപ്പെടാനുള്ള വിവരങ്ങൾ',
  'subsection.personalDetails': 'വ്യക്തിഗത വിവരങ്ങൾ',
  'subsection.membershipStatus': 'അംഗത്വ നില',
  'subsection.locationDetails': 'സ്ഥാന വിവരങ്ങൾ',

  // Form Labels
  'form.name': 'പേര്',
  'form.dob': 'ജനനതീയതി',
  'form.gender': 'ലിംഗം',
  'form.houseName': 'വീടിന്റെ പേര്',
  'form.pradeshikaSabha': 'പ്രദേശിക സഭ',
  'form.photo': 'ഫോട്ടോ',
  'form.photoHint': 'ഫോട്ടോ അപ്‌ലോഡ് ചെയ്യാൻ ക്ലിക്ക് ചെയ്യുക',
  'form.bloodGroup': 'രക്തഗ്രൂപ്പ്',
  'form.occupation': 'തൊഴിൽ',
  'form.membership': 'അംഗത്വം',
  'form.education': 'ഉയർന്ന വിദ്യാഭ്യാസം',
  'form.holdsSpssPosition': 'SPSS-ൽ എന്തെങ്കിലും പദവി വഹിക്കുന്നുണ്ടോ?',
  'form.holdsSpssPositionMember': 'അംഗം SPSS-ൽ എന്തെങ്കിലും പദവി വഹിക്കുന്നുണ്ടോ?',
  'form.spssPositionName': 'പദവിയുടെ പേര്',
  'form.address1': 'വിലാസം വരി 1',
  'form.address2': 'വിലാസം വരി 2',
  'form.place': 'സ്ഥലം',
  'form.pin': 'പിൻ കോഡ്',
  'form.healthInsurance': 'നിങ്ങളുടെ കുടുംബത്തിന് ആരോഗ്യ ഇൻഷുറൻസ് ഉണ്ടോ?',
  'form.termLifeInsurance': 'നിങ്ങൾക്ക് ടേം/ലൈഫ് ഇൻഷുറൻസ് ഉണ്ടോ?',
  'form.livingOutsideKerala': 'കേരളത്തിന് പുറത്ത് താമസിക്കുന്നുണ്ടോ?',
  'form.livingOutsideKeralaMember': 'അംഗം കേരളത്തിന് പുറത്ത് താമസിക്കുന്നുണ്ടോ?',
  'form.outsideReason': 'കാരണം',
  'form.phone': 'ഫോൺ',
  'form.phoneHint': 'സാധുവായ മൊബൈൽ നമ്പർ ഇല്ലെങ്കിൽ, ദയവായി 0 നൽകുക',
  'form.phoneHintMember': 'അംഗത്തിന് സാധുവായ മൊബൈൽ നമ്പർ ഇല്ലെങ്കിൽ, ദയവായി 0 നൽകുക',
  'form.email': 'ഇമെയിൽ',
  'form.relationship': 'വീട്ടുടമയുമായുള്ള ബന്ധം',
  'form.reasonNoMembership': 'അംഗത്വമില്ലാത്തതിന്റെ കാരണം',
  'form.areaOfExpertise': 'വൈദഗ്ധ്യ മേഖല',
  'form.rationCardType': 'റേഷൻ കാർഡ് നിറം',
  'form.selectOption': 'തിരഞ്ഞെടുക്കുക...',
  'form.noMembers': 'അംഗങ്ങൾ ഇതുവരെ ചേർത്തിട്ടില്ല. "അംഗം ചേർക്കുക" ക്ലിക്ക് ചെയ്യുക.',
  'form.noNonMembers': 'അംഗമല്ലാത്തവർ ഇതുവരെ ചേർത്തിട്ടില്ല. "അംഗമല്ലാത്തയാളെ ചേർക്കുക" ക്ലിക്ക് ചെയ്യുക.',

  // Options — Gender
  'option.male': 'പുരുഷൻ',
  'option.female': 'സ്ത്രീ',
  'option.other': 'മറ്റുള്ളവ',

  // Options — Yes/No
  'option.yes': 'അതെ',
  'option.no': 'ഇല്ല',

  // Options — Ration Card Color
  'option.rationNone': 'റേഷൻ കാർഡ് ഇല്ല',
  'option.rationWhite': 'വെള്ള',
  'option.rationYellow': 'മഞ്ഞ',
  'option.rationBlue': 'നീല',
  'option.rationPink': 'പിങ്ക്',

  // Options — Outside Reasons
  'option.job': 'ജോലി',
  'option.study': 'പഠനം',

  // Options — Occupation
  'option.centralGovt': 'കേന്ദ്ര സർക്കാർ ജീവനക്കാരൻ',
  'option.stateGovt': 'സംസ്ഥാന സർക്കാർ ജീവനക്കാരൻ',
  'option.privateEmployee': 'സ്വകാര്യ ജീവനക്കാരൻ',
  'option.selfEmployed': 'സ്വയം തൊഴിൽ',
  'option.kazhakam': 'കഴകം',
  'option.homemaker': 'വീട്ടമ്മ',
  'option.retired': 'വിരമിച്ചു',
  'option.unemployed': 'തൊഴിൽരഹിതൻ',
  'option.student': 'വിദ്യാർത്ഥി',

  // Options — Membership
  'option.lifeMember': 'ആജീവനാന്ത അംഗം',
  'option.ordinaryMember': 'സാധാരണ അംഗം',

  // Options — Education
  'option.below10th': '10-ാം ക്ലാസിന് താഴെ',
  'option.tenth': '10-ാം ക്ലാസ്',
  'option.plusTwo': 'പ്ലസ് ടു',
  'option.diploma': 'ഡിപ്ലോമ',
  'option.bachelors': 'ബിരുദം',
  'option.masters': 'ബിരുദാനന്തര ബിരുദം',
  'option.doctorate': 'ഡോക്ടറേറ്റ്',
  'option.professional': 'പ്രൊഫഷണൽ',
  'option.otherEdu': 'മറ്റുള്ളവ',

  // Options — Pradeshika Sabha
  'option.ernakulam': 'എറണാകുളം',
  'option.edappally': 'എടപ്പള്ളി',
  'option.tripunithura': 'തൃപ്പൂണിത്തുറ',
  'option.chottanikkara': 'ചോറ്റാനിക്കര',
  'option.perumbavoor': 'പെരുമ്പാവൂർ',
  'option.aluva': 'ആലുവ',
  'option.panangad': 'പനങ്ങാട്',

  // Options — Relationship to House Owner
  'option.spouse': 'ഭാര്യ/ഭർത്താവ്',
  'option.son': 'മകൻ',
  'option.daughter': 'മകൾ',
  'option.father': 'അച്ഛൻ',
  'option.mother': 'അമ്മ',
  'option.brother': 'സഹോദരൻ',
  'option.sister': 'സഹോദരി',
  'option.daughterInLaw': 'മരുമകൾ',
  'option.sonInLaw': 'മരുമകൻ',
  'option.grandchild': 'പേരക്കുട്ടി',
  'option.otherRelation': 'മറ്റുള്ളവ',

  // Buttons
  'btn.submit': 'രേഖ സേവ് ചെയ്യുക',
  'btn.update': 'രേഖ അപ്ഡേറ്റ് ചെയ്യുക',
  'btn.cancel': 'റദ്ദാക്കുക',
  'btn.addMember': 'അംഗം ചേർക്കുക',
  'btn.addNonMember': 'അംഗമല്ലാത്തയാളെ ചേർക്കുക',
  'btn.removeMember': 'നീക്കം ചെയ്യുക',

  // Dynamic Block
  'block.member': 'അംഗം',
  'block.nonMember': 'അംഗമല്ലാത്തയാൾ',

  // Validation
  'validation.nameRequired': 'പേര് ആവശ്യമാണ്',
  'validation.dobRequired': 'ജനനതീയതി ആവശ്യമാണ്',
  'validation.genderRequired': 'ലിംഗം ആവശ്യമാണ്',
  'validation.houseNameRequired': 'വീടിന്റെ പേര് ആവശ്യമാണ്',
  'validation.sabhaRequired': 'പ്രദേശിക സഭ ആവശ്യമാണ്',
  'validation.bloodGroupRequired': 'രക്തഗ്രൂപ്പ് ആവശ്യമാണ്',
  'validation.occupationRequired': 'തൊഴിൽ ആവശ്യമാണ്',
  'validation.membershipRequired': 'അംഗത്വ തരം ആവശ്യമാണ്',
  'validation.educationRequired': 'വിദ്യാഭ്യാസം ആവശ്യമാണ്',
  'validation.address1Required': 'വിലാസം വരി 1 ആവശ്യമാണ്',
  'validation.address2Required': 'വിലാസം വരി 2 ആവശ്യമാണ്',
  'validation.placeRequired': 'സ്ഥലം ആവശ്യമാണ്',
  'validation.pinInvalid': 'സാധുവായ 6 അക്ക പിൻ നൽകുക',
  'validation.phoneRequired': 'ഫോൺ നമ്പർ ആവശ്യമാണ്',
  'validation.phoneInvalid': 'സാധുവായ 10 അക്ക ഫോൺ നമ്പർ നൽകുക',
  'validation.emailInvalid': 'സാധുവായ ഇമെയിൽ വിലാസം നൽകുക',
  'validation.rationCardRequired': 'റേഷൻ കാർഡ് നിറം ആവശ്യമാണ്',
  'validation.positionNameRequired': 'ദയവായി പദവിയുടെ പേര് നൽകുക',
  'validation.outsideReasonRequired': 'ദയവായി ഒരു കാരണം തിരഞ്ഞെടുക്കുക',
  'validation.required': 'ഈ ഫീൽഡ് ആവശ്യമാണ്',
  'validation.dobFuture': 'ജനനതീയതി ഭാവിയിലായിരിക്കരുത്',

  // Messages
  'msg.saveSuccess': 'രേഖ വിജയകരമായി സേവ് ചെയ്തു!',
  'msg.updateSuccess': 'രേഖ വിജയകരമായി അപ്ഡേറ്റ് ചെയ്തു!',
  'msg.deleteSuccess': 'രേഖ വിജയകരമായി ഡിലീറ്റ് ചെയ്തു!',
  'msg.saveFailed': 'രേഖ സേവ് ചെയ്യാനായില്ല. വീണ്ടും ശ്രമിക്കുക.',
  'msg.photoUploading': 'ഫോട്ടോ അപ്‌ലോഡ് ചെയ്യുന്നു...',

  // Success Page
  'success.title': 'നന്ദി!',
  'success.message': 'നിങ്ങളുടെ രേഖ വിജയകരമായി സേവ് ചെയ്തു.',
  'success.note': 'താഴെയുള്ള ലിങ്ക് സൂക്ഷിക്കുക — നിങ്ങളുടെ വിവരങ്ങൾ കാണാനോ മാറ്റം വരുത്താനോ ഇത് എപ്പോൾ വേണമെങ്കിലും ഉപയോഗിക്കാം.',
  'success.urlLabel': 'നിങ്ങളുടെ എഡിറ്റ് ലിങ്ക്',
  'success.copy': 'കോപ്പി',
  'success.urlHint': 'ഈ ലിങ്ക് വിശ്വസനീയരായ ആളുകളുമായി മാത്രം പങ്കിടുക.',
  'success.addAnother': 'മറ്റൊരു രേഖ ചേർക്കുക',
  'success.goHome': 'ഹോം പേജിലേക്ക്',
};

export default ml;
