...
import { ENABLE_SPREADSHEET_SYNC, SPREADSHEET_API_URL } from '../constants/constants.js';
 ...
 * @sideeffects Network request; debug log of opaque response type when enabled.
 */
export async function sendToSpreadsheetApi(payload) {
  try {
    ...
    Logger.debug('Spreadsheet request sent (no-cors), response type:', res.type);
  } catch (err) {
    Logger.error('Spreadsheet request failed:', err);
  }
}
 ...
 * @sideeffects Optional fetch; optional debug log.
 */
export async function saveToSpreadsheet(recordId, formData) {
  ...
  if (!pradeshikaSabha) {
    Logger.warn('Spreadsheet sync skipped: pradeshikaSabha missing');
    return;
  }
  const payload = buildSpreadsheetPayload(recordId, formData, pradeshikaSabha);
  if (!payload || !payload.head) {
    Logger.warn('Spreadsheet sync skipped: invalid payload');
    return;
  }
  try {
    await sendToSpreadsheetApi(payload);
  } catch (err) {
    Logger.error('Spreadsheet save request failed:', err);
  }
}
...
export async function updateInSpreadsheet(recordId, formData) {
  ...
  try {
    ...
    Logger.error('Spreadsheet update request failed:', err);
  }
}
...
export async function deleteFromSpreadsheet(recordId, pradeshikaSabha) {
  ...
  if (!recordId || !pradeshikaSabha) {
    Logger.warn('Spreadsheet delete skipped: recordId or pradeshikaSabha missing');
    return;
  }
  const payload = {
    action: 'delete',
  ...
  try {
    ...
    Logger.error('Spreadsheet delete request failed:', err);
  }
}
