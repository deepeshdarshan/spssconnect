/**
 * @fileoverview Backup & Sync Center page controller.
 * @module pages/backup-sync-center-page
 */
...
/**
 * Updates sync/retry button enabled state based on config and running flag.
 */
function updateButtonStates() {
  ...
  if (syncBtn) {
    syncBtn.disabled = syncRunning || !configured;
  }
  if (retryBtn) {
    retryBtn.disabled = syncRunning || !configured;
  }

  const warning = document.getElementById('backupSyncConfigWarning');
  if (warning) {
    warning.classList.toggle('d-none', configured);
  }
}
...
/**
 * Initializes the Backup & Sync Center page.
 * @returns {Promise<void>}
 */
export async function initBackupSyncCenterPage() {
  if (!isSuperAdmin()) {
